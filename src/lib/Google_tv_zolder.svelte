<script lang="ts">
	...
	function handleClickgoogletv(service: string, entity: string) {
		callService($connection, 'media_player', service, {
			entity_id: entity
		});
	}
...
{#if isOpen}
	<Modal>
		...
		<div class="vol-container">
			{#if entity_id === 'media_player.zolderkamer_google_tv'}			
			<!-- volume_up -->
			<button on:click={() => handleClickgoogletv('volume_up','media_player.zolderkamer_google_tv_2')} use:Ripple={$ripple}>
